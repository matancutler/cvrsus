  ...
  async function addCvs(files) {
    ...
    try {
      /* Chunked like the builder's upload, and for the same reason: a dropped
         connection should cost one chunk rather than the whole pile. */
      let added = 0
      for (let at = 0; at < files.length; at += CHUNK) {
        const body = new FormData()
        for (const file of Array.from(files).slice(at, at + CHUNK)) body.append('cvs', file)
        // eslint-disable-next-line no-await-in-loop
        const data = await sendForm(`/api/hr/triage/${id}/cvs`, body)
        added += data.added ?? 0
    ...
    <div className="triage-page">
      ...
      {state?.limits?.capped && (
        <p className="alert alert-warning" role="status">
          Your organization has reached today&rsquo;s analysis limit, so new CVs are being scored
          by requirement matching rather than read in full. It resets on a rolling 24-hour basis.
        </p>
      )}
