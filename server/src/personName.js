/**
 * A person's name, capitalised the way names are written. Server side.
 *
 * The same rules as client/src/personName.js, and the two are deliberately
 * kept in step: the client tidies a Triage name parsed out of a CV for
 * display, and this one tidies what actually gets stored. Duplicated rather
 * than shared because the client is bundled by Vite and reaching into
 * server/ from it would drag the server's module graph into the browser.
 *
 * If you change one, change the other. test:identity checks they agree.
 */
